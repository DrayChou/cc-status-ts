...
function getPluginDir(homeDir: string): string {
  return path.join(getClaudeConfigDir(homeDir), 'plugins', 'cc-status-hud');
}

function getCachePath(homeDir: string): string {
  return path.join(getPluginDir(homeDir), '.speed-cache.json');
}
...
export function getOutputSpeed(stdin: StdinData, overrides: Partial<SpeedTrackerDeps> = {}): number | null {
  ...
  const deps: SpeedTrackerDeps = { ...defaultDeps, ...overrides };
  const now = deps.now();
